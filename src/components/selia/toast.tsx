      ...
      <BaseToast.Viewport className='fixed top-2 md:top-4 right-0 left-0 mx-2 md:mx-auto flex'>
        {toasts.map((toast) => (
          <BaseToast.Root
            ...
            swipeDirection='up'
            className={cn(
              'text-sm absolute right-0 top-0 left-0 z-[calc(1000-var(--toast-index))] mx-auto w-sm max-w-full origin-top',
...
function ToastContent({ toast }: { toast: ToastObject<Object> }) {
  return (
    <BaseToast.Content
      ...
      className={cn('overflow-hidden transition-opacity data-behind:pointer-events-none data-behind:opacity-0', 'data-expanded:opacity-100 data-expanded:pointer-events-auto', 'flex gap-x-2.5 gap-y-0.5 items-center')}
    >
      ...
      <div className='w-full flex justify-between flex-col md:flex-row items-start'>
        ...
        <BaseToast.Action
          ...
          className={cn('text-sm md:ml-auto shrink-0 mt-2 md:mt-0 self-center', buttonVariants({ variant: 'tertiary', size: 'xs' }), toast.actionProps?.className)}
        />
      </div>
    </BaseToast.Content>
  );
...
const icons: Record<string, React.ReactNode> = {
  loading: (
    <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' className='size-4.5 animate-spin'>
